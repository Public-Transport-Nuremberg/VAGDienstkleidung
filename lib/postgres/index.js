...
const listAllUsersByPage = async (offset, limit, sort, order, search) => {
  try {
    const validSortColumns = ['username', 'email', 'first_name', 'last_name', 'credit'];
    ...
    const totalItemsResult = await pool.query(countQuery, [searchPattern]);
...
const listInventory = async (offset, limit, sort, order, search) => {
  try {
    ...
    const inventoryResult = await pool.query(query, params);
